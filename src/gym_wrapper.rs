...
fn encode_pokemon_slot(
    ...
    let total_hp = pc.total_hp.max(1) as f32;
    let ko_pts   = pc.card.get_knockout_points() as f32;

    out[0] = 1.0;
    out[1] = pc.remaining_hp as f32 / total_hp;
    ...
    out[58] = if pc.poisoned  { 1.0 } else { 0.0 };
    out[59] = if pc.burned    { 1.0 } else { 0.0 };
    out[60] = if pc.paralyzed { 1.0 } else { 0.0 };
    out[61] = if pc.asleep    { 1.0 } else { 0.0 };
    out[62] = if pc.confused  { 1.0 } else { 0.0 };
    // out[63] reserved = 0
}
            ...
            if let Some(active) = state.maybe_get_active(agent_player) {
                if let Some(atk) = active.get_attacks().get(*atk_idx) {
                    ...
                    if let Some(opp_active) = state.maybe_get_active(opp) {
                        let opp_total = opp_active.total_hp.max(1) as f32;
                        let opp_rem   = opp_active.remaining_hp as f32;
...
impl PocketGym {
        ...
        -> PyResult<(Py<PyArray1<f32>>, f32, bool, bool, Py<PyDict>)>
    {
        ...
        loop {
            ...
            if let Some(ref policy) = self.opponent_policy {
                ...
                let chosen: usize = {
                    let obs_py  = opp_obs.into_pyarray_bound(py);
                    let mask_py = opp_mask.into_pyarray_bound(py);
    ...
    fn action_masks<'py>(&self, py: Python<'py>) -> Py<PyArray1<i8>> {
        build_mask(&self.possible_actions, self.max_actions).into_pyarray_bound(py).unbind()
    }
    ...
    fn build_observation<'py>(&self, py: Python<'py>) -> PyResult<Py<PyArray1<f32>>> {
        ...
        Ok(build_obs_for_player(state, self.agent_player, &self.possible_actions)
            .into_pyarray_bound(py).unbind())
    }
