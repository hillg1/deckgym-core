...
lazy_static! {
    static ref GLOBAL_STATE_SET: Mutex<HashSet<State>> = Mutex::new(HashSet::new());
}

pub fn reset_global_state_tracking() {
    GLOBAL_STATE_SET.lock().unwrap().clear();
}

pub fn get_global_unique_state_count() -> usize {
    GLOBAL_STATE_SET.lock().unwrap().len()
}
...
impl<'a> Game<'a> {
    ...
    pub fn play(&mut self) -> Option<GameOutcome> {
        ...
        while !self.state.is_game_over() {
            GLOBAL_STATE_SET.lock().unwrap().insert(self.state.clone());
            self.play_tick();
        }
